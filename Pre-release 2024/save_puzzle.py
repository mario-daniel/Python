...
#-----------------------------------------------------------------------------------------------------------------
    def __SavePuzzle(self, Filename):
        ...
        for Index in range(0, self.__GridSize**2):
            Symbol = ''
            SymbolsNotAllowed = ''
            if self.__Grid[Index].IsEmpty():
                pass
            else:
                Symbol = self.__Grid[Index].GetSymbol()
            for SymbolNotAllowed in self.__Grid[Index].GetSymbolsNotAllowed():
                SymbolsNotAllowed += SymbolNotAllowed
            f.write(f'{Symbol},{SymbolsNotAllowed}\n')
...
#-----------------------------------------------------------------------------------------------------------------
            SaveInput = input('Would you like to save the current puzzle? (y/N): ').lower()
            if SaveInput == 'y':
