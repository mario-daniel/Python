...
#----------------------------------------------------------------------------------------------------------------------    
    def __ReShuffleBlockedCells(self):
        ...
        for C in self.__Grid:
            if C.GetSymbol() == '@':
                BlockedCellsIndex.append(self.__Grid.index(C))
                BlockedCells.append(C)
        for Index in BlockedCellsIndex:
            self.__Grid[Index] = Cell()
        grid_position = random.randint(0, len(self.__Grid))
        while True:
            if grid_position not in BlockedCellsIndex and self.__Grid[grid_position].IsEmpty() and len(BlockedCells) != 0:
                self.__Grid[grid_position] = BlockedCells[-1]
                BlockedCells.pop()
                grid_position = random.randint(0, len(self.__Grid))
            elif len(BlockedCells) == 0:
                break
            else:
                grid_position = random.randint(0, len(self.__Grid))
#----------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------------------
                            UserInput = input('Would you like to reshuffle blocked cells? (y/N): ')
                            if UserInput == 'y':
