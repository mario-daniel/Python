...
#----------------------------------------------------------------------------------------------------------------------
            if CurrentCell.CheckSymbolAllowed(Symbol) and not CurrentCell.IsPattern():
                ...
                AmountToAddToScore = self.CheckforMatchWithPattern(Row, Column, False, CurrentCell)
            elif CurrentCell.GetPatternNumber() != 0 and not CurrentCell.IsEmpty():
                AmountToAddToScore = self.CheckforMatchWithPattern(Row, Column, True, CurrentCell)
                CurrentCell.ChangeSymbolInCell(Symbol)
...
#----------------------------------------------------------------------------------------------------------------------
    ...
    def CheckforMatchWithPattern(self, Row, Column, IsRemove, CurrentCell):
        if IsRemove == False:
...
#----------------------------------------------------------------------------------------------------------------------
                                self.__LatestPatternNumber += 1
                                for C in Pattern:
                                    if not C.IsEmpty():
                                        C.SetPatternNumber(self.__LatestPatternNumber)
...
#----------------------------------------------------------------------------------------------------------------------
            ...
            PatternNumber = CurrentCell.GetPatternNumber()
            for C in self.__Grid:
                if C.GetPatternNumber() == PatternNumber:
                    C.SetPatternNumber(0)
                    C.ChangePatternStatus(False)
                    C.RemoveFromNotAllowedSymbols(CurrentCell.GetSymbol())
            return -10
#----------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------------------
    ...
    def GetPatternNumber(self):
        return self.__PatternNumber

    def SetPatternNumber(self, PatternNumber):
        self.__PatternNumber = PatternNumber

    def RemoveFromNotAllowedSymbols(self, SymbolToRemove):
        if not self.CheckSymbolAllowed(SymbolToRemove):
